...
int main(){
    ...
    _tcscpy(now.lfFaceName, _T("微软雅黑"));
    
    settextstyle(&now);
    settextcolor(WHITE);
    drawtext(_T("唤醒！"), &r1, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
    drawtext(_T("退出"), &r2, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
    setlinecolor(WHITE);
		...
		switch(m.message)
		{
			...
			case WM_LBUTTONDOWN:
				...
				if (m.x>=30&&m.x<=150&&m.y>=30&&m.y<=70){
                    ...
                    settextstyle(&now);
                    settextcolor(WHITE);
                    drawtext(_T("已唤醒！"), &r1, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
